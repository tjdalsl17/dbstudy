...
SELECT * 
  FROM STUDENT_TBL S INNER JOIN PROFESSOR_TBL P
    ON S.P_NO = P.P_NO;
...
-- 교수번호, 교수이름, 교수전공, 강의이름, 강좌위치를 조회하시오.
...
SELECT S.S_NO, S.S_NAME, C.C_NAME
  FROM STUDENT_TBL S INNER JOIN ENROLL_TBL E
    ON S.S_NO = E.S_NO INNER JOIN COURSE_TBL C --COURSE_TBL = PK
    ON E.C_NO = C.C_NO;
...
-- 모든 교수들의 교수이름, 교수전공, 강의이름을 조회하시오. (교수는 총 3명)
