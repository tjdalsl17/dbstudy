...
-- 7. 제품을 구매한 이력이 있는 고객의 아이디, 고객명, 구매횟수, 총구매액을 조회하시오. (조인 + GROUP BY)
-- 아이디  고객명  구매횟수  총구매액
-- LHJ     이휘재  2         80
-- KYM     김용만  1         200
-- KHD     강호동  3         1210
...
SELECT U.USER_NAME AS 고객명
     , P.PROD_NAME AS 제품명
  FROM USERS U LEFT OUTER JOIN BUYS B
    ON U.USER_ID = B.USER_ID LEFT OUTER JOIN PRODUCTS P -- (구매하지 X 포함) 사용자 정보 전체  + 제품 테이블(LEFT OUTER JOIN)
    ON B.PROD_CODE = P.PROD_CODE;
...
SELECT U.USER_NAME AS 고객명
    , (SELECT P.PROD_NAME
         FROM PRODUCTS P
         WHERE P.PROD_CODE = B.PROD_CODE)  -- 구매내역과 제품내역 일치할 때
  FROM USERS U LEFT OUTER JOIN BUYS B
    ON U.USER_ID = B.USER_ID;
