...
SELECT EMP_NO, NAME, DEPART, GENDER, POSITION, HIRE_DATE, SALARY
 ...
 WHERE POSITION IN (SELECT POSITION
                     FROM EMPLOYEE_TBL -- 서브쿼리의 WHERE절에서 사용한 칼럼이 PK나 UNIQUE 칼럼이 아니므로 다중 행 서브쿼리이다.
                    WHERE DEPART = 2); -- 따라서 단일 행 연산자인 등호(=) 대신 다중 행 연산자 IN을 사용해야 한다.
                    
-- 7. 부서명이 '영업부'인 부서에 근무하는 사원을 조회하시오.
SELECT EMP_NO, NAME, DEPART, GENDER, POSITION, HIRE_DATE, SALARY
  FROM EMPLOYEE_TBL
 WHERE DEPART = (SELECT DEPART_NO
                   FROM EMPLOYEE_TBL
...
SELECT EMP_NO, NAME
  FROM EMPLOYEE_TBL
 WHERE ROWID = 'AAAE8DAABAAALC5AAA';
...
SELECT ROWNUM AS RN, ENP_NO, NAME
  FROM EMPLOYEE_TBL
...
--    3) 원하는 행 번호를 조회한다.
 ...
 WHERE E.RN BETWEEN 3 AND 4; -- WHERE E.RN = 3 OR E.RN = 4; 방법 다양함
